...
import { GetCoreBlocksQuerySchema, CreateCoreBlockRequestSchema } from '~/lib/requests';
...
export async function GET(request: NextRequest) {
    try {
        ...
        const coreBlocksList = await db.select()
            .from(coreBlocks)
            .where(and(...conditions));

        return Response.json({
            success: true,
            message: coreBlocksList.length > 0 ? "Core blocks retrieved successfully" : "No core blocks found",
...
export async function POST(request: NextRequest) {
    try {
        ...
        const newCoreBlock = await db.insert(coreBlocks)
            .values(parsedBody)
            .returning();
...
export async function DELETE(request: NextRequest) {
    try { 
        ...
        const deletedCoreBlocks = await db.delete(coreBlocks)
        .where(and(...conditions))
        .returning();

        return Response.json({
            success: true,
            message: deletedCoreBlocks ? `Core blocks for day of the week ${queryParams.dayOfWeek} and shift of the day ${queryParams.shiftOfDay} have been deleted` : "No changes made",
