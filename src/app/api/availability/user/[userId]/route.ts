...
import { GetUserAvailabilityQuerySchema, CreateAvailabilityRequestSchema } from "~/lib/requests/availability";
...
export async function GET(request: NextRequest, { params } : { params: { userId: string } }) {
    try {
        ...
        const availabilitiesList = await db
            ...
            .where(
                and(
                    eq(availabilities.userId, userIdInt),
                    ...conditions
                )
            )
        ...
        return Response.json({
            success: true,
            message: availabilitiesList.length > 0 ? "User availability retrieved successfully" : "No availability found for user",
...
export async function POST(request: NextRequest, { params } : { params: { userId: string } }) {
    try {
        ...
        if (!body || Object.keys(body).length === 0 || body.availability.length === 0) {
        ...
        const availabilityData = CreateAvailabilityRequestSchema.parse(bodyWithUserIds);
        
        // Handle duplicates by using ON_CONFLICT
        const newAvailabilities = await db.insert(availabilities)
            .values(availabilityData.availability) 
            .onConflictDoNothing() // Skip duplicates
            .returning();
        
        const insertedCoreIds = new Set(newAvailabilities.map((entry) => entry.coreId));
        const skippedEntries = availabilityData.availability.filter((entry) => !insertedCoreIds.has(entry.coreId));

        const isPartialSuccess = newAvailabilities.length > 0 && skippedEntries.length > 0;
        const isCompleteSuccess = newAvailabilities.length > 0 && skippedEntries.length === 0;
...
export async function PUT(request: NextRequest, { params } : { params: { userId: string } }) {
    try {
        ...
        const updatedAvailabilities = await db.insert(availabilities)
            .values(availabilityData.availability)
            .returning();

        return Response.json({
            success: true,
            message: `Updated ${updatedAvailabilities.length} availability records successfully`,
...
export async function DELETE(request: NextRequest, { params } : { params: { userId: string } }) {
    try {
        ...
        const deletedCount = await db.delete(availabilities)
