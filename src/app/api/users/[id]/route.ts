...
export async function PUT(request: NextRequest, { params }: { params: { id: string } }) {
    try {
        ...
        return Response.json({
            success: true,
            message: updatedUser ? `User with ID ${userId} has been updated` : "No changes made",
            users: updatedUser[0] || null,
            error: null,
        }, {status: 200});
    }  catch (error) {
        const apiError = handleAPIError(error);
...
export async function DELETE(request: NextRequest, { params }: { params: { id: string } }) {
    try {
        ...
        return Response.json({
            success: true,
            message: deletedUser ? `User with ID ${userId} has been deleted` : "No user found to delete",
            users: deletedUser[0] ?? null,
            error: null,
        }, {status: 200});
    } catch (error) {
        const apiError = handleAPIError(error);
